...
R4: 
## Characterizing the difference between Stan/PyMC3 and TensorFlow/PyTorch

What, if anything, are the hard differences between Bayesian modeling and deep learning? Deep learning can be implemented in Bayesian modeling, and lots of Bayesian concepts get used in the deep learning world as one reviewer noted:
...
* Unless the phenomenon under study is a physical neural net, deep learning only offers prediction services. While outstanding progress has been made and further progress may well involve Bayesian concepts, it is only prediction in the end. 
...
* Opacity blocks use in fields "dominated by Stats & Applied Math" where the goal involves developing and fitting mechanistic models. The science is in the model description, quality of fit validates the model. A high quality fit in the absence of an understandable model does not help. 
...
Another case are hybrid models where a deep learning component replaces some or all of the likelihood of a Bayesian model. While the overall model may be interpretable, the individual deep learning components remain black boxes. Interpretability comes from have well understood properties from how they were trained etc. For example Maggie Lieu's presentation at StanCon that used a deep learning component to substitute for a numerically unstable galaxy mass estimator().
...
In the game of "one of these things is not like the other", human interpretable models distinguishes Stan and PyMC3 from PyTorch and TensorFlow in quite absolute terms[^3]. 

If I am correct that deep learning and Bayesian modeling are so different when it comes to interpretablity then the mingling of the fields baffles the mind. They share similar programming techniques, have similar research leading edges and ... . 
I think Stan got a hold of the AI tail which whipped it out of booring stats into ML. 

[^3]:However, in a year this article may be much more difficult to write since both projects have adopted HMC implementations but I'd suggest that is more of a grafting of another approach to their code base rather than natural growth of deep learning techniques into the PyMC3/Stan space. 
...
The above queries can be run with the advance search option at https://scopus.com and are generated from the API queries actually used where '+' was replaced with ' ' above. An account is required which is included for many education institutions. 
...
R1: This paper also demonstrates that deep learning is not neccessarily the default tool when it is time for some reasoning in the scientific enterprise. If you are doing immunology/microbiology then Bayesian methods dominate. Environmental science, economics are a coin flip. Researchers use what is fashionable perhaps as a default--it may ease publication, be more familiar to students/postdocs but at the potential cost of being a bad fit for the problem at hand. If Bayesian methods are good enough for immunologists then perhaps other fields should have a look.:R1
